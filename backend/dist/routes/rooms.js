...
const createRoomSchema = zod_1.z.object({
    ...
    settings: zod_1.z.object({
        ...
        isRecordingEnabled: zod_1.z.boolean().optional(),
    }).optional(),
});
...
router.post("/:id/invite", auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
        ...
        const inviteToken = (0, crypto_1.randomBytes)(6).toString("hex");
...
router.get("/:id/participants", auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
        // Verify room ownership
        ...
        const { data: participants, error } = await supabase_1.supabase
            .from("participants")
            .select("*")
            .eq("room_id", id)
...
// End broadcast
router.post("/:id/end", auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
        const { data: room, error } = await supabase_1.supabase
